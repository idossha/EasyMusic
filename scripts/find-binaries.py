...
def main():
    """Main function."""
    print("🔍 Finding binaries that need code signing...")

    try:
        binaries = find_binaries_to_sign()

        if not binaries:
            print("❌ No binaries found to sign")
            return 1

        print(f"\n✅ Found {len(binaries)} binaries to sign")
        ...
        print(f"📝 Wrote {len(binaries)} binaries to {output_file}")
